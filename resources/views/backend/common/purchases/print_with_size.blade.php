        ...
        <div class="panel panel-bd lobidrag">
            ...
            <div id="printArea">
                ...
                <div class="panel-body">
                    ...
                    <h5 style="text-align: center">
                        <img style="height:100px;width:auto" src="{{ asset(@$purchase->store->logo)}}" alt="printing logo"
                                class="card-img-top">
                    </h5>
                    ...
                    <div class="row">
                        <div class="col-md-6" style="width: 60%; float: left;display: inline-block;">
                            Invoice No:{{ @$purchase->id }}<br>
                            Date & Time:</span> {{$dateTime= @$purchase->created_at }}<br>
                            Store:  {{ Helper::getStoreName(@$purchase->store_id) }}<br>
                            Sup. Name:{{ @$purchase->supplier->name }}<br>
                            Previous Due: {{$previousDue}} {{ $default_currency->symbol }} <br>
                            Total Due: {{$previousDue+@$purchase->due_amount}} {{ $default_currency->symbol }}
                        </div>
                    ...
                    <table class="invoice">
                        ...
                        @foreach($stocks as $key => $stock)
                            <tr>
                                ...
                                <td style="text-align: left">
                                    {{ Str::limit(@$stock->product->name, 50, '..') }}<br/>
                                </td>
                                <td>{{@$stock->unit->name}}</td>
                                <td>{{@$stock->qty}}</td>
                                <td style="text-align: right">{{number_format(@$stock->purchase_price,2)}}</td>
                                <td style="text-align: right">
                                    {{number_format(@$stock->qty*@$stock->purchase_price,2)}}
                                </td>
                            </tr>
                        @endforeach
                        <tr>
                        ...
                        <tr>
                            ...
                            <td colspan="3">&nbsp;</td>
                            <td style="text-align: right">{{ $default_currency->symbol }} {{number_format(@$purchase->sub_total,2)}}</td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align: left"> VAT :</td>
                            <td colspan="3">&nbsp;</td>
                            <td style="text-align: right">{{ $default_currency->symbol }} {{number_format(@$purchase->total_vat,2)}}</td>
                        </tr>
                        <tr>
                            <td colspan="2" style="text-align: left">Discount:</td>
                            <td colspan="3">&nbsp;</td>
                            <td style="text-align: right">{{ $default_currency->symbol }} {{number_format(@$purchase->discount,2)}}</td>
                        </tr>
                        {{-- <tr>
                            <td colspan="2" style="text-align: left">After Discount Amount:</td>
                            <td colspan="3">&nbsp;</td>
                            <td style="text-align: right">{{ $default_currency->symbol }} {{number_format(@$purchase->after_discount,2)}}</td>
                        </tr> --}}

                        <tr>
                            <th colspan="2" style="text-align: left"> Grand Total:</th>
                            <th colspan="3">&nbsp;</th>
                            <th style="text-align: right">{{ $default_currency->symbol }} {{number_format(@$purchase->grand_total,2)}}</th>
                        </tr>
                        </tbody>
                    </table>
                    <table style="width: 100%;margin-right:auto;padding-top: 5px;" class="sm-padding small strong pt-2">
                        <tbody>
                            <tr>
                                <th class="text-right strong" style="text-align: right">In Word :{{ucwords($digit->format($purchase->grand_total,2))}} Only</th>
                            </tr>
                        </tbody>
                    </table>
...
<body>
    <div class="main-invoice">
        ...
        <div style="float: left;display: inline-block;">
            ...
            Date & Time  <span dir="ltr" lang="AR">:</span> {{$dateTime= @$purchase->created_at }}<br>
            Store:  {{ Helper::getStoreName(@$purchase->store_id) }}<br>
            Cust. Name :{{ @$purchase->supplier->name }}<br>
            Previous Due: {{$previousDue}} <br>
            Total Due: {{$previousDue+@$purchase->due_amount}}
        </div>
        @php
        ...
        //     new InvoiceDate(@$purchase->created_at),
        ...
        @foreach ($stocks as $stock)
            <div class="d-flex" style="margin-top: 10px;">
                ...
                <p style="width: 35%; font-size:10px">
                    {{ Str::limit(@$stock->product->name, 50, '..') }} <br>
                    {{ Str::limit(@$stock->product->arabic_name, 50, '..') }}
                </p>
                <p style="width:10%">
                   {{@$stock->unit->name}}
                </p>
                <p style="width:25%" class="text-center">
                    {{ @$stock->outer_sale_price }}
                </p>
        ...
        <div style="margin-left: auto; width: 95%; margin-top: 10px;">
            ...
            <div class="d-flex">
                ...
                <div class="w-50" style="text-align: right">
                    <div class="border"></div>
                    <p style="padding-bottom: 5px;">
                        {{ $default_currency->symbol }} {{ @$purchase->sub_total }}
                    </p>

                    <p style="padding-bottom: 5px;">
                        {{ $default_currency->symbol }} {{ @$purchase->total_vat }}
                    </p>
                    <p style="padding-bottom: 5px;">
                        {{ $default_currency->symbol }} {{ @$purchase->discount }}
                    </p>
                </div>
            </div>
            <div class="border"></div>
            <div class="d-flex">
                ...
                <div class="w-50" style="text-align: right">
                    <p style="padding-bottom: 5px;">
                        {{ $default_currency->symbol }} {{ @$purchase->grand_total }}
                    </p>
            ...
            <div class="d-flex">
                ...
                <div class="w-50" style="text-align: right">
                    <p style="padding-bottom: 5px;">
                        {{ $default_currency->symbol }} {{ @$purchase->paid }}
                    </p>
...
{{--            <h1 style="">--}}
{{--                <img width="170mm" height="30mm" src="data:image/png;base64,{!! DNS1D::getBarcodePNG(@$purchase->id, 'C39') !!}" />--}}
{{--            </h1>--}}
{{--        </div>--}}
